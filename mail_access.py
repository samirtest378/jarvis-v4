"""
JARVIS Mail Access — read, search and send through Apple Mail or Outlook.

Accounts already configured in Mail.app on macOS or classic Outlook on Windows
are automatically available. No separate JARVIS OAuth setup is needed.

Reading is unrestricted. Writing is limited to sending a new message:
existing mail is never edited, moved or deleted, because nothing here can
undo that and the user has no way to see it happened.
"""
...
from windows_outlook import outlook_messages, run_outlook_action
...
async def _run_native_outlook(action: str, **kwargs) -> dict:
    global _native_available, _native_error
    payload = await run_outlook_action(action, **kwargs)
    _native_available = bool(payload.get("available"))
    _native_error = str(payload.get("error") or "")[:240]
    return payload


def native_mail_status() -> dict[str, object]:
    return {
        "available": _native_available,
        "source": "outlook" if sys.platform == "win32" else "apple-mail" if sys.platform == "darwin" else "",
        "error": _native_error,
    }
...
async def get_accounts() -> list[str]:
    ...
    if sys.platform == "win32":
        payload = await _run_native_outlook("recent", limit=1)
        account = str(payload.get("account") or "Outlook").strip()
        return [account] if payload.get("available") else []
    script = """
...
async def get_unread_count() -> dict:
    ...
    """
    if sys.platform == "win32":
        payload = await _run_native_outlook("unread", limit=5)
        if not payload.get("available"):
            return {"total": 0, "accounts": {}, "available": False, "source": "outlook"}
        total = max(0, int(payload.get("total") or 0))
        account = str(payload.get("account") or "Outlook")[:120]
        return {"total": total, "accounts": {account: total}, "available": True, "source": "outlook"}
    script = """
tell application "Mail"
...
async def get_recent_messages(count: int = 10) -> list[dict]:
    ...
    """
    if sys.platform == "win32":
        return outlook_messages(await _run_native_outlook("recent", limit=count))
    script = f"""
tell application "Mail"
...
async def get_unread_messages(count: int = 10) -> list[dict]:
    ...
    if sys.platform == "win32":
        return outlook_messages(await _run_native_outlook("unread", limit=count))
    script = f"""
tell application "Mail"
...
async def search_mail(query: str, count: int = 10) -> list[dict]:
    ...
    """
    if sys.platform == "win32":
        return outlook_messages(await _run_native_outlook("search", query=query, limit=count))
...
async def read_message(subject_match: str) -> dict | None:
    ...
    """
    if sys.platform == "win32":
        payload = await _run_native_outlook("read", query=subject_match, limit=1)
...
def parse_recipients(text: str) -> list[str]:
    """Pull email addresses out of spoken or written text.

    Dictation writes addresses in several shapes — "samir at example dot com",
    "samir@example.com", "<samir@example.com>" — so the spoken forms are
    normalised before matching.
    """
    spoken = re.sub(r"\s+(?:at|ät)\s+", "@", text, flags=re.IGNORECASE)
    spoken = re.sub(r"\s+(?:dot|punkt)\s+", ".", spoken, flags=re.IGNORECASE)
    spoken = spoken.replace(" @ ", "@")
    seen: list[str] = []
    for match in _ADDRESS.findall(spoken):
        address = match.strip(".,;:")
        if address.lower() not in {existing.lower() for existing in seen}:
            seen.append(address)
    return seen


async def send_mail(to: str | list[str], subject: str, body: str) -> dict:
    """Send a new message through Apple Mail or classic Outlook.

    Returns {"success": bool, "confirmation": str}. Sending is only reported
    as done once the native mail application accepts the message.
    """
    recipients = parse_recipients(to if isinstance(to, str) else ", ".join(to))
    if not recipients:
    ...
    if sys.platform == "win32":
        result = await _run_native_outlook(
            "send",
        ...
        if result.get("available") and result.get("sent") is True:
            log.info("Sent Outlook mail to %s", ", ".join(recipients))
            return {
                "success": True,
        ...
        return {
            "success": False,
            "confirmation": "I couldn't send that, sir — classic Outlook is unavailable or refused the message.",
        }

    recipient_lines = "\n".join(
        f'        make new to recipient at end of to recipients with properties {{address:"{_escape_applescript(address)}"}}'
...
"""
    result = await _run_mail_script(script, timeout=45)
    if result.strip() != "sent":
        log.warning("Mail send failed for %s", ", ".join(recipients))
        return {
            "success": False,
            "confirmation": "I couldn't send that, sir — Mail refused the message.",
        }
...
def format_unread_summary(unread: dict, language: str = "en") -> str:
    ...
    if len(parts) == 1:
        if german:
            return f"Sie haben {total} ungelesene {'Nachricht' if total == 1 else 'Nachrichten'} — {parts[0]}."
        return f"You have {total} unread {'message' if total == 1 else 'messages'} — {parts[0]}."
    elif parts:
        return (
            f"Sie haben {total} ungelesene Nachrichten: {', '.join(parts)}."
            if german else f"You have {total} unread messages: {', '.join(parts)}."
        )
    else:
        return (
            f"Sie haben {total} ungelesene {'Nachricht' if total == 1 else 'Nachrichten'}."
            if german else f"You have {total} unread {'message' if total == 1 else 'messages'}."
        )
...
def format_messages_for_voice(messages: list[dict], language: str = "en") -> str:
    """Format messages for voice response."""
    german = language == "de"
    if not messages:
        return "Keine Nachrichten vorhanden." if german else "No messages to report, sir."

    count = len(messages)
    if count == 1:
        m = messages[0]
        sender = _short_sender(m["sender"])
        return f"Eine Nachricht von {sender}: {m['subject']}." if german else f"One message from {sender}: {m['subject']}."

    summaries = []
    for m in messages[:5]:
        sender = _short_sender(m["sender"])
        summaries.append(f"{sender} wegen {m['subject']}" if german else f"{sender} regarding {m['subject']}")

    result = f"Sie haben {count} Nachrichten. " if german else f"You have {count} messages. "
