"""
JARVIS Calendar Access — read Apple Calendar or classic Outlook Calendar.
...
from windows_outlook import classic_outlook_installed, outlook_events, run_outlook_action
...
async def refresh_cache():
    ...
    global _event_cache, _cache_time, USER_CALENDARS, _auto_discovered, _native_available, _native_error
    if sys.platform == "win32":
        payload = await run_outlook_action("calendar", limit=100)
        _native_available = bool(payload.get("available"))
        _native_error = str(payload.get("error") or "")[:240]
        _event_cache = outlook_events(payload)
        _event_cache.sort(key=lambda event: (not event["all_day"], event.get("start_dt") or datetime.max))
        _cache_time = _time.time()
        if _native_available:
            log.info("Outlook calendar refreshed: %d events today", len(_event_cache))
        return
    if sys.platform != "darwin":
        _native_available = False
        _native_error = "No native calendar bridge is available on this platform."
        _event_cache = []
        _cache_time = _time.time()
        return
...
async def get_todays_events() -> list[dict]:
    ...
    if not _event_cache and (_cache_time == 0 or _time.time() - _cache_time > 60):
...
async def get_calendar_names() -> list[str]:
    ...
    if sys.platform == "win32":
        return ["Outlook"] if classic_outlook_installed() else []
    await _ensure_calendar_running()
    try:
        proc = await asyncio.create_subprocess_exec(
            "osascript", "-e",
...
def native_calendar_status() -> dict[str, object]:
    """Report the last native bridge result without triggering an OS prompt."""
    return {
        "available": _native_available,
        "source": "outlook" if sys.platform == "win32" else "apple-calendar" if sys.platform == "darwin" else "",
        "error": _native_error,
    }
...
def format_schedule_summary(events: list[dict], language: str = "en") -> str:
    """Format a brief voice-friendly summary of the schedule."""
    german = language == "de"
    if not events:
        return "Ihr Kalender ist heute frei." if german else "Your schedule is clear today, sir."
    ...
    if count == 1:
        ...
        if evt.get("all_day"):
            return (
                f"Sie haben einen ganztägigen Termin: {evt['title']}."
                if german else f"You have one all-day event: {evt['title']}."
            )
        return (
            f"Sie haben einen Termin: {evt['title']} um {evt['start']}."
            if german else f"You have one event: {evt['title']} at {evt['start']}."
        )

    summaries = []
    for evt in events[:5]:
        if evt.get("all_day"):
            summaries.append(f"{evt['title']} ganztägig" if german else f"{evt['title']} all day")
        else:
            summaries.append(f"{evt['title']} um {evt['start']}" if german else f"{evt['title']} at {evt['start']}")

    result = f"Sie haben heute {count} Termine. " if german else f"You have {count} events today. "
