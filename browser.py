...
def _launch_headless() -> bool:
    """Use a hidden browser only when there is no interactive desktop.

    JARVIS keeps browser work visible during normal desktop use. Automated
    builds and Linux sessions without a display still need Chromium's headless
    mode, otherwise the browser exits before the first page opens.
    """
    override = os.getenv("JARVIS_BROWSER_HEADLESS", "").strip().casefold()
    if override:
        return override in {"1", "true", "yes", "on"}
    if os.getenv("CI", "").strip():
        return True
    return sys.platform.startswith("linux") and not (
        os.getenv("DISPLAY", "").strip() or os.getenv("WAYLAND_DISPLAY", "").strip()
    )
...
class JarvisBrowser:
    ...
    async def _ensure_browser(self):
        ...
        # Prefer Playwright Chromium, then use a supported system browser. This
        # keeps browser automation optional instead of crashing when the large
        # Playwright download was intentionally skipped.
        launch_attempts = [{}, {"channel": "chrome"}, {"channel": "msedge"}]
        system_browser = (
        ...
        if system_browser:
            launch_attempts.append({"executable_path": system_browser})
        headless = _launch_headless()
        last_error = None
        for options in launch_attempts:
            try:
                self._browser = await self._pw.chromium.launch(headless=headless, **options)
                break
        ...
        if not self._browser:
            ...
            self._pw = None
            raise RuntimeError(
                "No supported browser is installed. Run 'playwright install chromium' "
        ...
        log.info("Browser launched (%s Chromium)", "headless" if headless else "visible")
